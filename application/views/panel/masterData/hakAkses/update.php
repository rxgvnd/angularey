        ...
        <div class="panel-body">
          ...
          <form class="form-horizontal" method="post" action="<?php echo base_url(changeLink('panel/masterData/updateHakAkses/doUpdate/' . $id)); ?>">
            <div class="form-group">
              ...
              <div class="col-md-10">
                <input type="text" class="form-control" placeholder="Masukkan Nama Level Pengguna" name="nama_hak_akses" value="<?php echo $hak_akses[0]->nama_hak_akses; ?>" required />
              </div>
            </div>
                ...
                <div class="panel panel-inverse overflow-hidden">
                  <div class="panel-heading">
                    <h3 class="panel-title">
                      ...
                      <input type="checkbox" name="class_parent_modul[]" value="<?php echo $key->class; ?>" onchange="cek('<?php echo $key->class; ?>')" class="pull-right" id="parent<?php echo $key->class; ?>" <?php if (cekParentModulPengguna($key->class, $id) == TRUE) {
                                                                                                                                                                                                                    echo "checked";
                                                                                                                                                                                                                  } ?>>
                    </h3>
                  </div>
                  <?php if ($key->child_module == 'Y') : ?>
                    <div id="<?php echo $key->class; ?>" class="panel-collapse collapse <?php if (cekParentModulPengguna($key->class, $id) == TRUE) {
                                                                                          echo "in";
                                                                                        } ?>">
                      <div class="panel-body">
                        <table class="table table-striped table-bordered">
                          ...
                          <tbody>
                            <?php
                            $modul = $this->GeneralModel->get_by_id_general_order_by('at_modul', 'class_parent_modul', $key->class,'urutan_modul','ASC');
                            foreach ($modul as $row) : ?>
                              <tr>
                                <?php if ($row->type_modul == 'R') : ?>
                                  <td style="background-color:green;color:white;"><input type="checkbox" name="controller_modul[]" value="<?php echo $row->controller_modul; ?>" <?php if (cekModulPengguna($row->controller_modul, $id) == TRUE) {
                                                                                                                                                                                    echo "checked";
                                                                                                                                                                                  } ?>></td>
                                ...
                                <?php else : ?>
                                  <td><input type="checkbox" name="controller_modul[]" value="<?php echo $row->controller_modul; ?>" <?php if (cekModulPengguna($row->controller_modul, $id) == TRUE) {
                                                                                                                                        echo "checked";
                                                                                                                                      } ?>></td>
